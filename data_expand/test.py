...
for key, indices in voxel_to_indices.items():
    ...
    if key in chosen_keys:
        ...
        original_points.append(selected)  # 元の点も残す
    else:
...
pcd_kept = numpy_to_pcd(kept, color=(0.5, 0.5, 0.5))
pcd_orig = numpy_to_pcd(original, color=(0.2, 0.8, 0.2))  # 元のFPS点：緑
pcd_noisy = numpy_to_pcd(noisy, color=(1.0, 0.0, 0.0))    # ノイズ後：赤

# ---------- 保存（ノイズ入りのみ） ----------
pcd_all = numpy_to_pcd(np.concatenate([kept, noisy], axis=0), color=(0.8, 0.5, 0.3))
out_path = OUT_DIR / "partial_voxel_fps_noise.pcd"
...
o3d.visualization.draw_geometries(
    ...
    window_name="Original (green) vs Noisy (red)",
    width=1024,
    height=768,
)
