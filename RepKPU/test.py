...
def upsampling(args, model, input_pcd):
    ...
    # print(f"patch_pts_num: {patch_pts_num}")
...
def test(model, args):
    with torch.no_grad():
        ...
        for i, path in enumerate(test_input_path):
            ...
            txt_result.append(f'{pcd_name}: CD={cd * 1e3:.4f}, Time={infer_time:.3f} sec')
            total_cd += cd
            counter += 1.0

        avg_cd = total_cd / counter * 1e3
        avg_time = total_time / counter
        txt_result.append(f'overall: {avg_cd:.4f}')
        txt_result.append(f'average_time: {avg_time:.3f} sec')
...
def test_flexible(model, args):
    with torch.no_grad():
        ...
        avg_cd = total_cd / max(counter, 1) * 1e3
        avg_time = total_time / max(counter, 1)
        txt_result.append(f'overall: {avg_cd:.4f}')
        txt_result.append(f'average_time: {avg_time:.3f} sec')
...
if __name__ == '__main__':
    ...
    en = time.time()
    print(f'Total time: {en - st} seconds')
    # print(f'The number of patches: {patch}')